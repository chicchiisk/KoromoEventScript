...
`.kel` の詳細文法が確定するまでは、次の最小ハイライトを提供する。
...
| タグ | `#id` 形式 |
| パス | `.ke` を指す相対パス |
| 識別子 | イベントID、エントリ名、属性名 |

`.kel` の文法が確定した時点で、予約語と構造ハイライトを追加する。
...
| カーソル位置 | ジャンプ先 |
|---|---|
| `.ke` ファイル参照 | 対応する `.ke` ファイル |
| イベントID参照 | 対応するイベント定義 |
| エントリポイント | 対応する `.ke` または `.kel` 内定義 |

`.kel` の文法が未確定の間は、ファイルパス参照のジャンプを優先実装とする。
