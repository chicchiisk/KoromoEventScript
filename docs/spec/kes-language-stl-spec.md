...
STL は `.ke` ファイルから暗黙に利用できる組み込み関数・命令群であり、ノベルゲーム MVP を成立させるために必要最小限の機能を提供する。
...
- `__systemcall__` は STL 実装と VM/runtime の境界であり、通常のシナリオ `.ke` から直接呼び出してはならない。
...
- プロジェクトの通常 `.ke` ファイルから `__systemcall__` を呼び出した場合はコンパイルエラーとする。
...
- `__systemcall__` が通常 `.ke` から直接呼び出された場合はコンパイルエラーとする。
- syscall ID が文字列リテラルでない場合はコンパイルエラーとする。
