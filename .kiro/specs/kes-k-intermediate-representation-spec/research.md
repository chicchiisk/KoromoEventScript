...
- **Key Findings**:
  - `docs/spec/cli-tool-spec.md` は `.klib` を build output として定義済みだが、命令体系とデータ構造は別仕様に委譲している。
  - `docs/spec/windows-runtime-spec.md` は manifest と VM 成果物を読む runtime 前提を持つが、`.kc` / `.klib` 旧称が残る。
  - 現在の C# 実装には lexer/parser/semantic diagnostics の基盤がある一方、`.klib` emitter と VM は未実装であるため、今回は implementation ではなく公開契約の先行定義に限定する。

## Research Log

### 既存 CLI build 契約

- **Context**: `.klib` ファイル形式の仕様化範囲を決めるため、既存 build output の所有者を確認した。
- **Sources Consulted**: `docs/spec/cli-tool-spec.md`
- **Findings**:
  - `kes build` は `.kc` / `.kel` を解析・検証し、`.kc` ごとに VM 向け `.klib` を生成する。
  - build output は `.klib`、`diagnostics.json`、`manifest.json` を含む。
  - CLI 仕様は `.klib` の命令体系、データ構造、形式詳細を別仕様に委譲している。
- **Implications**: `.klib` 詳細契約は CLI 仕様へ埋め込まず、独立した `docs/spec/k-intermediate-representation-spec.md` として追加する。

### Runtime と manifest 契約

- **Context**: `.klib` が所有する情報と manifest が所有する情報の境界を明確にする必要がある。
...
- **Implications**: `.klib` は script execution unit と instruction/value/source mapping を所有し、manifest は成果物一覧、entry、asset/locale/runtime metadata を所有する。旧称は互換性注記で扱う。

### 言語/STL/flow 語彙

- **Context**: `.klib` instruction schema が表現すべき source language と runtime call の語彙を確認した。
...
- **Implications**: `.klib` 仕様では source syntax ではなく VM instruction と operand の契約としてこれらを表現する。`.kel` entry/chapter と `.klib` 開始位置の対応は manifest と `.klib` の参照契約として定義する。
...
- **Findings**:
  ...
  - `.klib` emitter、VM、runtime は未実装であり、既存実装の format を追認する段階ではない。
...
|--------|-------------|-----------|---------------------|-------|
| A | 独立した `.klib` 中間表現仕様を追加する | CLI、VM、runtime、debug tooling の共通参照点になる。詳細 schema を分散させない | 既存仕様への cross-reference 更新が必要 | 選択する |
| B | CLI 仕様の build セクションに `.klib` 詳細を埋め込む | `kes build` 利用者には見つけやすい | VM、runtime、debug/source map、manifest との関係まで CLI 仕様に入り責務が肥大化する | 選択しない |
| C | `.klib` 仕様と manifest schema 仕様を同時に分割追加する | runtime contract をより厳密にできる | Issue 範囲を超え、asset manifest や locale schema まで過剰に固定する | 選択しない |

## Design Decisions

### Decision: `.klib` は独立した公開仕様として定義する

- **Context**: CLI 仕様が `.klib` 詳細を別仕様へ委譲しており、VM/runtime が参照できる共通契約が必要である。
- **Alternatives Considered**:
  1. CLI 仕様に詳細 schema を追加する。
  2. runtime 仕様に VM input schema を追加する。
  3. 独立した `.klib` 中間表現仕様を追加する。
...
### Decision: `.kc` / `.klib` を現行の正とする

- **Context**: 既存文書には `.kc` / `.klib` が残る一方、CLI 仕様は `.kc` / `.klib` を現行成果物として定義している。
- **Alternatives Considered**:
  1. 既存文書全体を今回一括置換する。
  2. 新仕様だけ `.kc` / `.klib` とし、既存文書は旧称のまま放置する。
  3. 新仕様で `.kc` / `.klib` を正とし、今回触る既存仕様に旧称注記を追加する。
- **Selected Approach**: `.kc` source と `.klib` intermediate representation を正とし、`.kc` / `.klib` は旧称または移行前の用語として注記する。
...
- **Context**: `.klib` は asset ID、locale key、script path を扱うが、manifest の完全 schema は隣接仕様の責務である。
- **Alternatives Considered**:
  1. `.klib` 仕様内で manifest schema も完全定義する。
  2. manifest への参照を一切書かない。
  3. `.klib` が持つ ID/key/path 参照と manifest 所有情報の境界だけ定義する。
- **Selected Approach**: `.klib` は manifest-owned data を複製せず、script id、asset id、locale key、script path などの参照契約を定義する。
- **Rationale**: runtime contract に必要な接続だけを定義し、manifest schema の固定化を避ける。
- **Trade-offs**: manifest 詳細 schema は後続仕様または runtime 仕様に残る。
- **Follow-up**: manifest schema を独立仕様化する場合は `.klib` 仕様との revalidation が必要になる。
...
- **Simplification**: manifest schema、VM interpreter、runtime save implementation、binary package format は本仕様から除外し、`.klib` の公開契約と cross-reference 更新に集中する。

## Risks & Mitigations

- Risk: `.klib` schema を早く固定しすぎると将来の VM 実装とずれる。
  - Mitigation: `version`、`features`、unsupported feature handling、revalidation triggers を仕様に含める。
- Risk: manifest と `.klib` の責務境界が曖昧になる。
  - Mitigation: `.klib` は script execution unit と VM operand を持ち、manifest は成果物一覧、entry、asset/locale/runtime metadata を所有する、と明記する。
- Risk: `.kc` / `.klib` 旧称が混在する。
  - Mitigation: 新仕様と今回触る既存仕様への注記で `.kc` / `.klib` を authoritative term とし、広範な用語統一は別作業に分離する。
