...
この仕様は、`.ke` から生成される VM 実行用中間表現 `.k` の公開契約を `docs/spec/k-intermediate-representation-spec.md` として追加する。
...
- `.k` の目的、拡張子、encoding、newline、version、feature compatibility、正規化例を定義する。
- VM が必要とする instruction、operand、value、variable、scope、execution state、control flow を表現できる契約を定義する。
- runtime error、debug 表示、golden test が元 `.ke` の位置を参照できる source mapping 方針を定義する。
- manifest が所有する情報と `.k` が所有または参照する情報の境界を明記する。
- 既存仕様に残る `.kc` / `.klib` と現行 `.ke` / `.k` の関係を注記する。

### Non-Goals

- `.k` emitter、serializer、schema validator の実装。
...
- `docs/spec/k-intermediate-representation-spec.md` を `.k` 中間表現の authoritative specification として追加する。
- `.k` document の top-level schema、instruction schema、value/reference model、control-flow representation、source mapping、manifest 参照、compatibility policy を定義する。
- `say`、`nar`、通常命令、式評価、変数定義、代入、`label`、`jump`、`select`、`case`、`__systemcall__` 相当の runtime call を VM が読める形で表現するための契約を定義する。
- `.k` が save data ではないことを前提に、save/load が参照できる script id、instruction index、call state、scope、variable state の識別情報を定義する。
- CLI、runtime、overview から新しい `.k` 仕様へ到達できる最小限の参照更新を定義する。

### Out of Boundary

- `.k` を生成または読み取る C# 実装は行わない。
- manifest が所有する entry、asset、locale、runtime、build metadata の完全 schema は本仕様で所有しない。
- `.kel` の構文、`.ke` 言語構文、STL syscall signature の詳細は既存仕様が所有する。
- Windows/Unity/Unreal runtime 固有の描画、音声、入力、保存先、UI、配布形式は本仕様で所有しない。
- `.kc` / `.klib` 旧称の全面的な用語移行は別作業に分離する。

### Allowed Dependencies

- `docs/spec/cli-tool-spec.md`: `kes build` / `kes run` / `kes publish` が `.k` と `manifest.json` を扱う成果物契約。
- `docs/spec/kes-language-spec.md`: `.ke` の構文、名前、型、変数、制御構文、source position の前提。
- `docs/spec/kes-language-stl-spec.md`: `__systemcall__`、STL、runtime call、asset ID、actor/tag の既存語彙。
- `docs/spec/kel-file-spec.md`: `.kel` entry/chapter 参照と `.ke` script path の前提。
...
- `.k` top-level field、instruction field、opcode、value/reference representation、source mapping shape を変更する場合。
- manifest が所有する entry/script/asset/locale 情報と `.k` が所有する情報の境界を変更する場合。
- `.ke` / `.k` と `.kc` / `.klib` の authoritative term を変更する場合。
- VM/runtime が未知 version、unsupported feature、manifest reference error を扱う期待動作を変更する場合。
- import された `.ke` の execution unit 化、entry label 解決、instruction index の安定性を変更する場合。

## Architecture

### Existing Architecture Analysis

`docs/spec/cli-tool-spec.md` は、`kes build` が `.ke` / `.kel` を解析し、`.ke` ごとに VM 向け `.k` を生成し、`manifest.json` とともに出力することを定義している。
一方で、中間表現の命令体系、データ構造、形式詳細は別仕様で定義する前提になっている。

`docs/spec/windows-runtime-spec.md` は、runtime が source script を直接実行せず、manifest と VM 成果物を読み込む構成を持つ。ただし既存文書には `.kc` / `.klib` という旧称が残るため、本仕様は `.ke` / `.k` を現行の正とし、旧称は移行上の注記として扱う。
...
- Selected pattern: 独立仕様による contract-first documentation。CLI、VM、runtime に詳細 schema を分散させず、`.k` 仕様を共通参照点にする。
- Domain/feature boundaries: `.k` は VM execution contract を所有し、manifest は成果物一覧、entry、asset、locale、runtime、build metadata を所有する。
...
- New components rationale: `docs/spec/k-intermediate-representation-spec.md` は未定義だった `.k` 詳細契約を埋めるために必要である。
...
|-------|------------------|-----------------|-------|
| Documentation | Markdown | `.k` 公開仕様と既存仕様参照を記述する | `docs/` 配下のため日本語で作成する |
| Contract format | JSON-style text example | `.k` 正規化例と schema 説明を表現する | 実装形式の固定ではなく、review/golden test の基準として扱う |
...
```text
docs/spec/
  k-intermediate-representation-spec.md  # 新規: .k IR の authoritative specification
  cli-tool-spec.md                       # 変更: kes build/run/publish から .k 仕様への参照を追加
  windows-runtime-spec.md                # 変更: .k 仕様参照と .kc/.klib 旧称注記を追加
  overview.md                            # 変更: 仕様一覧または概要に .k 仕様と旧称注記を追加
...
- `docs/spec/k-intermediate-representation-spec.md` — `.k` の file format、schema、instruction、value、control flow、source mapping、manifest relation、compatibility、minimal sample を定義する。
- `docs/spec/cli-tool-spec.md` — `kes build` の `.k` 出力説明から新仕様へ参照を張り、CLI 仕様が詳細 schema を所有しないことを明確にする。
- `docs/spec/windows-runtime-spec.md` — runtime が読む VM 成果物として `.k` 仕様を参照し、`.kc` / `.klib` が旧称であることを注記する。
- `docs/spec/overview.md` — 詳細仕様一覧または workflow 説明に `.k` 中間表現仕様を追加し、現行 `.ke` / `.k` と旧称の関係を読者が追えるようにする。

## Requirements Traceability

| Requirement | Summary | Components | Interfaces | Flows |
|-------------|---------|------------|------------|-------|
| 1.1 | `.k` の目的、拡張子、encoding、newline、version、互換性方針 | KIntermediateRepresentationSpec | K Document contract | Build to VM artifact |
| 1.2 | 単一 `.ke` と複数ファイル project の扱い | KIntermediateRepresentationSpec | Module/import contract | Build to VM artifact |
...
| 2.5 | import された `.ke` の実行単位への反映 | KIntermediateRepresentationSpec | Module/import contract | Build to VM artifact |
...
| 5.1 | manifest が `.k` を列挙または参照する script 情報 | KIntermediateRepresentationSpec, CliSpecReferenceUpdate, RuntimeSpecTerminologyNote | Manifest reference contract | Manifest to VM flow |
| 5.2 | `.kel` entry/chapter と `.k` 開始位置の対応 | KIntermediateRepresentationSpec, RuntimeSpecTerminologyNote | Entry/start position contract | Manifest to VM flow |
...
| 5.4 | manifest 所有情報と `.k` 所有情報の境界 | KIntermediateRepresentationSpec | Ownership boundary contract | Manifest to VM flow |
...
| 6.2 | `.ke` / `.k` と `.kc` / `.klib` の扱い | KIntermediateRepresentationSpec, RuntimeSpecTerminologyNote, OverviewIndexUpdate | Terminology contract | Documentation navigation |
...
|-----------|--------------|--------|--------------|------------------|-----------|
| KIntermediateRepresentationSpec | Documentation / Contract | `.k` IR の authoritative contract を定義する | 1.1-6.4 | CLI spec P0, language/STL specs P0, runtime specs P1 | State, Batch |
| CliSpecReferenceUpdate | Documentation / CLI | CLI build output から `.k` 仕様へ導線を張る | 5.1, 6.1, 6.4 | CLI spec P0, KIntermediateRepresentationSpec P0 | Batch |
| RuntimeSpecTerminologyNote | Documentation / Runtime | runtime 入力と旧称を `.k` 仕様へ接続する | 3.3, 5.1-5.4, 6.1, 6.2, 6.4 | Windows runtime spec P0, KIntermediateRepresentationSpec P0 | State |
| OverviewIndexUpdate | Documentation / Navigation | overview から `.k` 仕様へ到達可能にする | 6.1, 6.2, 6.4 | overview P1, KIntermediateRepresentationSpec P0 | Batch |
...
- Inbound: CLI / VM / runtime / debug tooling — `.k` contract の参照 (P0)
...
- Trigger: `kes build` が `.ke` / `.kel` を解析し、`.k` と `manifest.json` を生成する。
- Input / validation: `.ke` の名前解決、型検査、tag/asset/locale 参照検証は `.k` 生成前に完了している前提を明記する。
- Output / destination: `.k` は `build/<target>/events/*.k` など CLI 仕様の出力先に配置され、manifest から参照される。
...
| Field | Detail |
|-------|--------|
| Intent | CLI 仕様が `.k` 詳細 schema を所有しないことを明示し、新仕様へ参照を張る |
| Requirements | 5.1, 6.1, 6.4 |

**Responsibilities & Constraints**

- `kes build` の成果物説明に `.k` 仕様への参照を追加する。
- CLI は `.k` を生成する責務を持つが、instruction schema の詳細定義は `KIntermediateRepresentationSpec` に委譲する。
...
- Output / destination: `.k` と `manifest.json` の出力関係を `.k` 仕様へリンクする。

#### RuntimeSpecTerminologyNote

| Field | Detail |
|-------|--------|
| Intent | runtime が読む VM 成果物を `.k` 仕様へ接続し、旧称との関係を注記する |
| Requirements | 3.3, 5.1, 5.2, 5.3, 5.4, 6.1, 6.2, 6.4 |

**Responsibilities & Constraints**

- Windows runtime 仕様の `.klib` 表記に対して、現行 `.k` 仕様を参照する互換性注記を追加する。
- save/debug に必要な VM file、instruction position、tag などは `.k` の script id / instruction index / source mapping を参照することを明確にする。
- 描画、音声、入力、保存先の runtime 固有詳細は変更しない。

**Dependencies**

- Inbound: Windows runtime spec — runtime 入力、save/debug の既存記述 (P0)
- Outbound: `KIntermediateRepresentationSpec` — `.k` と source mapping の authoritative source (P0)

**Contracts**: Service [ ] / API [ ] / Event [ ] / Batch [ ] / State [x]

##### State Management

- State model: save/debug は `.k` の script id、instruction index、label/tag、source mapping を参照する。
- Persistence & consistency: `.k` 自体を save data とせず、save data は `.k` 上の安定識別子を参照する。
...
| Field | Detail |
|-------|--------|
| Intent | overview から `.k` 仕様を発見できるようにする |
| Requirements | 6.1, 6.2, 6.4 |

**Responsibilities & Constraints**

- overview の workflow または仕様一覧に `.k` 中間表現仕様を追加する。
- `.kc` / `.klib` 表記が残る場合は、現行仕様では `.ke` / `.k` を正とする注記を追加する。
...
- `imports`: import 済み `.ke` module の参照情報。
...
- `.k` は `say` / `nar` 本文、LESS 展開、`select` / `case` のように 1 つの source construct が複数命令へ展開される場合、各命令の primary source と related source range の方針を定義する。
- `.k` は `__systemcall__` 相当の runtime call に syscall ID、typed args、return value usage を含める。
- `.k` は manifest の asset ID、locale key、script path を値または `manifestRefs` として参照し、manifest 側の詳細 metadata を複製しない。
...
Build 時点で名前、型、tag、syscall signature、manifest reference の静的検証を完了する。Runtime/VM は `.k` load 時に version、features、instruction schema、manifest reference を検証し、実行中は instruction index と source mapping を debug/error 表示へ利用する。

## Error Handling

### Error Strategy

- 未知の major version は `.k` load error として読み込み失敗にする。
...
- Contract errors: `.k` schema、opcode、operand、result が仕様に合わない場合は load 時点で失敗する。
...
- `.k` の正規化例が `format`、`version`、`features`、`module`、`instructions`、`labels`、`manifestRefs`、`debug` を含むことを確認する。
...
- `.kc` / `.klib` が残る箇所について、今回触る既存仕様に `.ke` / `.k` への互換性注記があることを確認する。
- manifest 所有情報と `.k` 所有情報が重複せず、ID/key/path 参照として説明されていることを確認する。

### Golden Test Preparation

- 最小 `.k` サンプルが stable field order と optional field policy を示し、将来の golden test で比較可能であることを確認する。
...
- `.k` 仕様は runtime が読む成果物契約を定義するが、署名、暗号化、改ざん検出は本仕様の対象外とする。
...
- `.k` は VM が sequential instruction execution と instruction index lookup を行える構造を持つ。
- label は runtime 解決ではなく build 時に instruction index へ解決済みとし、実行時 jump/select の探索負荷を抑える。
- import された `.ke` の扱いは module/import contract として定義し、複数ファイル project でも execution unit の境界を追跡できるようにする。

## ADR Consideration

今回の設計判断は公開仕様追加の範囲であり、C# 実装アーキテクチャ、永続化実装、binary format、圧縮・暗号化方式を固定しない。
ただし `.k` format を emitter/VM 実装で採用する Issue では、互換性方針、schema validator、save/load reference stability を ADR 化するか再検討する。
